...
export default function Terms() {
  ...
  return (
    ...
    >
      ...
      <p>{t('termsBody.pAbout') || 'Budva Car Hire (budvacarhire.com) is an affiliate website that connects travellers with car rental services in Budva, Montenegro. We are not a car rental company, we partner with LocalRent to provide real-time availability and booking services.'}</p>

      <h2>{t('termsBody.h2Booking') || 'Booking & Rental Terms'}</h2>
      <p>{t('termsBody.pBooking1') || 'All car rental bookings made through our website are processed and fulfilled by our partner, LocalRent. When you complete a booking, you enter into a rental agreement directly with the vehicle supplier, not with Budva Car Hire.'}</p>
      ...
      <p>{t('termsBody.pLiabilityIntro') || 'Budva Car Hire acts as an intermediary and is not liable for:'}</p>
      <ul>
      ...
      <p>{t('termsBody.pIP') || 'All content on this website, including text, design, and graphics, is owned by Budva Car Hire unless otherwise credited. Stock photography is used under licence from Pexels.'}</p>

      <h2>{t('termsBody.h2Contact') || 'Contact'}</h2>
      <p>{t('termsBody.pContact') || 'Questions about these terms? Contact us at info@budvacarhire.com.'}</p>
    </ContentPage>
  );
}
