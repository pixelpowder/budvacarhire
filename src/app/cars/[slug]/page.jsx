...
export async function generateMetadata({ params }) {
  ...
  const title = meta?.title || `${car.name} Hire in Budva | Budva Car Hire`;
  const description = meta?.desc || `${car.name} rental guide for Budva and the Montenegrin Riviera.`;
...
export default async function CarDetailRoute({ params }) {
  ...
  const vehicleSchema = {
    ...
    'image': `https://www.budvacarhire.com${car.image}`,
