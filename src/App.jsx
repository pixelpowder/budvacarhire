    ...
    <section className="section" id="beaches">
      <div className="container">
        ...
        <div className="beach-grid">
          {[
            { title: t('beachGuide.jazTitle') || 'Jaz Beach', desc: t('beachGuide.jazDesc') || 'Open sand bay 3 km north of town. Free parking in the upper lot, paid near the water. Summer music festival venue.', distance: '3 km', time: t('beachGuide.jazTime') || '5 min drive', image: '/img/jaz-beach.webp' },
            { title: t('beachGuide.mogrenTitle') || 'Mogren Beach', desc: t('beachGuide.mogrenDesc') || 'Twin coves connected by a tunnel carved through the cliff. Walking distance from the Old Town walls. Loungers and bar May to September.', distance: '0.5 km', time: t('beachGuide.mogrenTime') || 'Walk', image: '/img/budva-beach.webp' },
            { title: t('beachGuide.beciciTitle') || 'Be\u010Di\u0107i Beach', desc: t('beachGuide.beciciDesc') || 'Long family-friendly sand strip with lifeguards in season. Gentle shallow entry, ideal for children. Ample paid parking.', distance: '2 km', time: t('beachGuide.beciciTime') || '5 min', image: '/img/becici-beach.webp' },
            { title: t('beachGuide.svetiStefanTitle') || 'Sveti Stefan', desc: t('beachGuide.svetiStefanDesc') || 'The iconic fortified island village. Public beach on the mainland side. Best photographed from the viewpoint above the road.', distance: '6 km', time: t('beachGuide.svetiStefanTime') || '10 min', image: '/img/sveti-stefan.webp' },
          ].map((beach) => (
    ...
    <section className="section fleet-showcase" id="fleet-info">
      <div className="container">
        <div className="section-header">
          ...
          <h2 className="section-title">{tf('fleetShowcase.title', 'Guides to every car in the Budva Riviera fleet')}</h2>
          <p className="section-subtitle">{tf('fleetShowcase.subtitle', 'Specs, fuel use, boot size and what each car is really good at on the M2 coast road, the Sveti Stefan hairpins and the Budva Old Town perimeter.')}</p>
        </div>
    ...
    <section className="section" id="blog">
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px', marginBottom: '32px' }}>
          {blogPosts.map((post) => (
            <a key={post.href} href={localePath(post.href)} style={{ background: '#fff', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--gray-200, #e9ecef)', boxShadow: 'var(--shadow-sm)', transition: 'box-shadow 0.2s, transform 0.2s', textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column' }}
              onMouseEnter={(e) => { e.currentTarget.style.boxShadow = 'var(--shadow-md)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.boxShadow = 'var(--shadow-sm)'; e.currentTarget.style.transform = 'translateY(0)'; }}
            >
              <img src={post.image} alt={t(`blogIndex.card_${post.key}_title`)} style={{ width: '100%', height: '200px', objectFit: 'cover', display: 'block' }} loading="lazy" />
              <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                <h3 style={{ fontSize: '17px', fontWeight: 800, color: 'var(--navy, #05203c)', lineHeight: 1.3, marginBottom: '8px' }}>{t(`blogIndex.card_${post.key}_title`)}</h3>
                <p style={{ fontSize: '14px', color: 'var(--gray-600, #6c757d)', lineHeight: 1.6, flex: 1 }}>{t(`blogIndex.card_${post.key}_excerpt`)}</p>
              </div>
            </a>
          ))}
        </div>
        <div style={{ textAlign: 'center' }}>
          <a href={localePath('/blog')} style={{ fontSize: '14px', fontWeight: 700, color: 'var(--blue, #0770e3)', textDecoration: 'none' }}>{t('blogHome.viewAll')} &rarr;</a>
        </div>
      </div>
    </section>
  );
